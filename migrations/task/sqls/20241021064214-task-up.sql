...
('透明人','opacity0@he xschooltest.io','USER');
...
delete from "USER" where email='opacity0@he xschooltest.io';
...
((select "COACH".id from "COACH" join  "USER" on "USER".id ="COACH".user_id
where "USER".email = 'lee2000@hexschooltest.io'),(select id from "SKILL" where name = '重訓')),
((select "COACH".id from "COACH" join  "USER" on "USER".id ="COACH".user_id
where "USER".email = 'muscle@hexschooltest.io'),(select id from "SKILL" where name = '重訓')),
((select "COACH".id from "COACH" join  "USER" on "USER".id ="COACH".user_id
where "USER".email = 'starplatinum@hexschooltest.io'),(select id from "SKILL" where name = '重訓')),
((select "COACH".id from "COACH" join  "USER" on "USER".id ="COACH".user_id
where "USER".email = 'muscle@hexschooltest.io'),(select id from "SKILL" where name = '瑜伽')),
((select "COACH".id from "COACH" join  "USER" on "USER".id ="COACH".user_id
where "USER".email = 'starplatinum@hexschooltest.io'),(select id from "SKILL" where name = '有氧運動')),
((select "COACH".id from "COACH" join  "USER" on "USER".id ="COACH".user_id
where "USER".email = 'starplatinum@hexschooltest.io'),(select id from "SKILL" where name = '復健訓練'));
-- 3-3 修改：更新教練的經驗年數，資料需求如下：
    -- 1. 教練`肌肉棒子` 的經驗年數為3年
    -- 2. 教練`Q太郎` 的經驗年數為5年
update "COACH" set experience_years = 3 where id =(select "COACH".id from "COACH" join "USER" on "USER".id ="COACH".user_id where "USER".email = 'muscle@hexschooltest.io');
update "COACH" set experience_years =5 where id =(select "COACH".id from "COACH" join  "USER" on "USER".id ="COACH".user_id where "USER".email = 'starplatinum@hexschooltest.io');
...
insert into "COURSE" 
(user_id,skill_id,name,start_at,end_at,max_participants,meeting_url) values
...
(
    ...
    (select "COURSE".id from "COURSE" join "USER" on "USER".id = "COURSE".user_id where "USER".email ='lee2000@hexschooltest.io'),'2024-11-24 16:00:00','即將授課'
);
    -- 2. 新增： `好野人` 預約 `李燕容` 的課程
        -- 1. 預約人設為 `好野人`
        -- 2. 預約時間`booking_at` 設為2024-11-24 16:00:00
...
update "COURSE_BOOKING" set cancelled_at ='2024-11-24 17:00:00',status ='課程已取消' where user_id = (select id from "USER" where email = 'wXlTq@hexschooltest.io');
...
insert into "COURSE_BOOKING" (user_id,course_id,booking_at,status)values
((select id from "USER" where email = 'wXlTq@hexschooltest.io'),
(select "COURSE".id from "COURSE" join "USER" on "USER".id = "COURSE".user_id where "USER".email ='lee2000@hexschooltest.io'),'2024-11-24 17:10:25','即將授課');
...
-- 5-5. 修改：`王小明` 現在已經加入直播室了，請在`COURSE_BOOKING`更新該筆預約資料（請注意，不要更新到已經取消的紀錄）：
    ...
    -- 2. 狀態`status` 設定為上課中
update "COURSE_BOOKING" set join_at ='2024-11-25 14:01:59',status ='上課中' where user_id = (select id from "USER" where email = 'wXlTq@hexschooltest.io') and cancelled_at isnull ;
-- 5-6. 查詢：計算用戶王小明的購買堂數，顯示須包含以下欄位： user_id , total。 (需使用到 SUM 函式與 Group By)
select "CREDIT_PURCHASE".user_id,sum("CREDIT_PACKAGE".credit_amount) from "CREDIT_PURCHASE" join "CREDIT_PACKAGE" on "CREDIT_PACKAGE".id = "CREDIT_PURCHASE".credit_package_id where  user_id = (select id from "USER" where email = 'wXlTq@hexschooltest.io') group by user_id;
-- 5-7. 查詢：計算用戶王小明的已使用堂數，顯示須包含以下欄位： user_id , total。 (需使用到 Count 函式與 Group By)
select user_id,count(*) from "COURSE_BOOKING"  where status ='上課中' and  user_id = (select id from "USER" where email = 'wXlTq@hexschooltest.io') group by user_id;
...
select "SKILL".name as 專長名稱,count("USER".name)as coach_total from "COACH"
inner join "COACH_LINK_SKILL" ON "COACH_LINK_SKILL".coach_id = "COACH".id
inner join "USER" ON "USER".id = "COACH".user_id
inner join "SKILL" ON "SKILL".id = "COACH_LINK_SKILL".skill_id
group by  "SKILL".id
...
-- 顯示須包含以下欄位： 總營收
select COALESCE(sum(price_paid), 0) as 總營收 from "CREDIT_PURCHASE"
where "CREDIT_PURCHASE".purchase_at between'2024-11-01 00:00:00' AND '2024-11-30 23:59:59';
-- 6-5. 查詢：計算 11 月份有預約課程的會員人數（需使用 Distinct，並用 created_at 和 status 欄位統計）
-- 顯示須包含以下欄位： 預約會員人數
select count(*) as 預約會員人數 from "COURSE_BOOKING" 
where status !='課程已取消' 
and	"COURSE_BOOKING".created_at between'2024-11-01 00:00:00' AND '2024-11-30 23:59:59';